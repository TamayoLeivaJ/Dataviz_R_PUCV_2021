...
style_duo_accent(
  ...
  title_slide_background_image = "./Image/Generative_Art_001.png",
...
<small> ¿Qué ocurre si ocupamos `color` y no `fill`?</small>
...
--
Todas estas tranformaciones son realizadas por elementos llamados _stat_ ( _**stat**istical transformation_). En el caso de las cuentas el elemnto se llama `stat_count()`
...
.right-plot[
```{r plot-label_11f-out, ref.label="plot-label_11f", echo=FALSE, fig.dim=c(9, 6)}
```
]
...
.center[
```{r plot-label_13c-out, ref.label="plot-label_13c", fig.show="hold", echo=FALSE, fig.dim=c(4.5, 3)}
```
]
...
.right-plot[
```{r plot-label_13d-out, ref.label="plot-label_13d", echo=FALSE, fig.dim=c(9, 6)}
```
]
...
.center[
Opciones de paletas de color <br><br><br> 
